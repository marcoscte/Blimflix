...
<html>
	<head>
		...
		try{
			...
			System.out.println("Se conecto topSeries: "+ usuario.getId());
			}catch(Exception e){
				e.printStackTrace();
				System.out.println("No se conecto topSeries");
		...
		<body background="bg.jpg">
	        ...
	        <h1 style="color:white;">TOP 10 SERIES CON MEJOR CALIFICACION</h1>
	        <%String queryTop = "SELECT VIEW_SERIES.TITULO_SERIE, VIEW_SERIES.PORTADA_SERIE, VIEW_SERIES.SINOPSIS_SERIE, "
				        		+"AVG(USUARIO_SERIE.CALIFICACION) AS CALIF FROM USUARIO_SERIE "
				        		+"INNER JOIN VIEW_SERIES ON VIEW_SERIES.CVE_SERIE = USUARIO_SERIE.CVE_SERIE "
				        		+"GROUP BY (USUARIO_SERIE.CVE_SERIE, VIEW_SERIES.TITULO_SERIE, "
			       				+" VIEW_SERIES.PORTADA_SERIE, VIEW_SERIES.SINOPSIS_SERIE) "
			       		   		+"ORDER BY CALIF DESC LIMIT 10;";
			     String titulo="";
			     String portada="";
			     String sinopsis="";
			     float avg = 0.0F;
			     int i = 0;
			       ...
			       try{
			    	   resultSet = statement.executeQuery(queryTop);
			    	   while(resultSet.next()){
			    	   	titulo = resultSet.getString("TITULO_SERIE");
			    	   	portada = resultSet.getString("PORTADA_SERIE");
			    	   	sinopsis = resultSet.getString("SINOPSIS_SERIE");
			    	   	avg = resultSet.getFloat("CALIF");
			    	   	i++;
			    	   	%>
			    	   ...
			    	   <div class="card" style="width: 19rem;">
						  ...
						  <li class="list-group-item">PUESTO: <%=i %></li>
						    <li class="list-group-item">CALIFICACION: <%=avg %> / 5</li>
						  </ul>
						</div>
						</div>
